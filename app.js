...
function runAIAnalysis(){const s=stocks[state.selected];const score=s.proofsToday*18+s.comments.length*6+(s.fv-s.price)*0.8-s.strikes*20+Math.random()*28;let type='neutral',txt='중립: 아직 결정적 성과가 부족해서 가격은 거의 유지돼.';if(score>40){type='good';txt='호재: 인증 성과와 크루 반응이 좋아 AI가 다음 장 기대치를 올렸어.'}else if(score<8){type='bad';txt='악재: 인증 공백이나 신뢰도 약화가 감지돼 단기 위험 프리미엄이 붙었어.'}document.getElementById('aiEventTone').textContent=type==='good'?'호재':type==='bad'?'악재':'중립';document.getElementById('aiEventText').textContent=txt;applyMarketEvent(type,txt,type==='good'?'호재':type==='bad'?'악재':'중립')}
